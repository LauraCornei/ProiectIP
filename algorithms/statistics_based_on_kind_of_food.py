...
# http://127.0.0.1:5000/recommendations/stats/food-per-restaurant/5eb17156251c5187bd95ff83?order=asc&token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1ZWFhOTQ2ODMwYThmMTI5OGQ0ZmMyZjgiLCJpYXQiOjE1ODgyMzc0NTZ9.Ll2HDuN79KKWr5OoQTiZVWBemyDqdo3kDz74Bvi6lOA
import Constants
...
def get_statistics_per_restaurant(restaurant_id, orders, sort_order, show_count):
    ...
    for order in orders:
        for item in order[Constants.ITEMS]:
            if restaurant_id == 0 or order[Constants.RESTAURANT_ID] == restaurant_id:
                food_dict[item[Constants.ITEM][Constants.PRODUCT]] = 0
                food_name_by_id[item[Constants.ITEM][Constants.PRODUCT]] = item[Constants.ITEM][Constants.PRODUCT]

    for order in orders:
        for item in order[Constants.ITEMS]:
            if restaurant_id == 0 or order[Constants.RESTAURANT_ID] == restaurant_id:
                food_dict[food_name_by_id[item[Constants.ITEM][Constants.PRODUCT]]] += item[Constants.ITEM][Constants.QUANTITY]
